...
if (typeof res == 'number') throw res == -1 ? `*[❗𝐈𝐍𝐅𝐎❗] 𝚃𝙷𝙴 𝙴𝙵𝙵𝙴𝙲𝚃𝚂 ${effect} 𝙸𝚃 𝙸𝚂 𝙽𝙾𝚃 𝙸𝙽 𝚃𝙷𝙴 𝙻𝙸𝚂𝚃 𝙾𝙵 𝙴𝙵𝙵𝙴𝙲𝚃𝚂*` : `*[❗𝐈𝐍𝐅𝐎❗] 𝚃𝙷𝙴 𝙲𝙾𝚁𝚁𝙴𝙲𝚃 𝚄𝚂𝙴 𝙾𝙵 𝚃𝙷𝙴 𝙲𝙾𝙼𝙼𝙰𝙽𝙳 𝙸𝚂 ${usedPrefix + command} ${effect} ${new Array(res).fill('texto').map((v, i) => v + (i ? i + 1 : '')).join('|')}*`
let result = await axios.get(res, {
responseType: 'arraybuffer'
})
await conn.sendFile(m.chat, result.data, 'Error.jpg', `*𝚃𝙰𝙺𝙴 𝚈𝙾𝚄𝚁 𝙲𝚄𝚂𝚃𝙾𝙼 𝙸𝙼𝙰𝙶𝙴!!*\n*𝙴𝙵𝙴𝙲𝚃𝙾: ${effect}*`, m)
}
handler.help = ['logos']
handler.tags = ['nulis']
